...
class Services {
  static Future<List<CellModel>> fetchHomeData() async {
    final response = await http.get(APPURLS.GENRE_URL);
    try {
      if (response.statusCode == 200) {
        List<CellModel> list = parsePostsForHome(response.body[0]);
...
class Endpoints {
  static String discoverMoviesUrl(int page) {
    return '$TMDB_API_BASE_URL/discover/movie?api_key=1d51cef38e888586924becd5f1cc9b1d&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=$page';
  }
  ...
  static String movieSearchId(String query) {
    return "https://api.themoviedb.org/3/movie/$query?api_key=1d51cef38e888586924becd5f1cc9b1d&language=en-US";
  }
